...
class InvMassGenerator {
  ...
  vector<double> m;//to store the masses of the n particles to decay plus the mass of the State s to decay
  TRandom3 ran;
  vector<double> Mmin;//lower bound for the invariant mass set
  vector<double> M;//invariant mass set to be returned
...
public:
  ...
  vector<double> GetMassSet(); //returns a copy of M without the first (mother) and the last elements (last child)
